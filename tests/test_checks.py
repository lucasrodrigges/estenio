...
def test_all_dependencies_present():
    ...
    with patch("estenio.checks.shutil.which", return_value="/usr/bin/found"):
        with patch("estenio.checks._get_ffmpeg_path", return_value=None):
...
def test_ffmpeg_bundled():
    """When ffmpeg is bundled, it's flagged as present with bundled=True."""
    def mock_which(cmd):
        if cmd == "yt-dlp":
            return "/usr/bin/yt-dlp"
        return None
    ...
    with patch("estenio.checks.shutil.which", side_effect=mock_which):
        with patch("estenio.checks._get_ffmpeg_path", return_value="/app/bin/ffmpeg-linux"):
            results = check_dependencies()
            ytdl = next(r for r in results if r.name == "yt-dlp")
            ffmpeg_r = next(r for r in results if r.name == "ffmpeg")
            assert ytdl.present
            assert not ytdl.bundled
            assert ffmpeg_r.present
            assert ffmpeg_r.bundled
            assert all_present(results)
